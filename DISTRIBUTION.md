# Distribution playbook (getting the site linked and seen)

Purpose: the site has 40+ pages but almost no inbound links, and Google will not rank a
domain nothing links to. This file holds ready-to-paste copy for the places worth listing
on and posting in. Copy from here, do not rewrite from scratch each time.

Facts to keep consistent everywhere (change them here first if they ever change):

- Price: ১০ টাকা/কেজি, ৫০ কেজি এয়ারটাইট বস্তা ৫০০ টাকা. Bulk discount negotiable.
- Daily feeding: দুধের গাভী ১৫-২৫ কেজি, মোটাতাজাকরণ ১০-২০ কেজি, ছাগল/ভেড়া ১-২ কেজি.
- Storage: unopened airtight bag ৬-১২ মাস; after opening use within ২-৩ দিন.
- Payment: cash on delivery, nationwide. Transport quoted per order on the confirmation call.
- WhatsApp: +880 1303-438063 · Website: https://silage.khamarvest.com
- Facebook: https://www.facebook.com/khamarvestSilage/ · YouTube: https://www.youtube.com/@khamarvest

Rule: never promise a delivery date, a guaranteed weight gain, or a profit figure. Those are
the claims that lose trust and that we cannot support.

---

## 1. Where to put the link first (free, do these once)

These are the fastest real backlinks and referral sources. Tick them off:

- [ ] Facebook page → About → Website field, and a pinned post that includes the link
- [ ] Every YouTube video description (first line, above the fold)
- [ ] YouTube channel → About → Links
- [ ] WhatsApp Business profile → Description / Website field
- [ ] Google Business Profile → Website field (see GMB_SETUP.md)
- [ ] Livestock1 supplier listing: https://www.livestock1.com/silage-suppliers/bangladesh
- [ ] TradeFord: https://bangladesh.tradeford.com/suppliers/corn-silage
- [ ] TradeKey Bangladesh corn silage category
- [ ] Any local agri Facebook group that allows a business listing in its pinned post

Note on the B2B directories: they are low-quality links on their own, but buyers genuinely
browse them for suppliers, and it costs nothing. Treat them as lead sources first, links second.

---

## 2. Directory listing copy

### Company name
Khamarvest Silage (খামারভেস্ট সাইলেজ)

### Category / keywords
Corn silage, maize silage, cattle feed, animal feed, green fodder, dairy feed, cattle
fattening feed, silage supplier Bangladesh

### Short listing (about 250 characters)
Khamarvest Silage supplies fresh corn (maize) silage to dairy and beef farms across
Bangladesh. Whole corn plants chopped at the correct maturity stage and naturally fermented
in 50 kg airtight bags. BDT 10/kg. Cash on delivery nationwide. WhatsApp +880 1303-438063.

### Long listing (about 900 characters)
Khamarvest Silage is a Bangladeshi supplier of corn (maize) silage for dairy cattle, beef
fattening cattle, goats and sheep. We harvest whole corn plants at the correct maturity
stage, chop them, and let them ferment naturally in airtight 50 kg bags, so farms get
green-fodder nutrition all year, including the winter and monsoon months when fresh grass
is scarce or risky to feed.

Price is BDT 10 per kg, packed in 50 kg airtight bags at BDT 500 per bag, with discounts
negotiable on bulk orders. An unopened bag keeps 6 to 12 months, and should be used within
2 to 3 days once opened. Typical daily feeding is 15 to 25 kg for a dairy cow, 10 to 20 kg
for fattening cattle and 1 to 2 kg for goats and sheep.

We deliver across Bangladesh with cash on delivery, including Bogura, Pabna, Sirajganj,
Natore, Rangpur, Dinajpur, Mymensingh, Tangail, Dhaka, Gazipur, Kushtia, Jashore, Khulna,
Satkhira, Cumilla, Noakhali, Chattogram and Cox's Bazar. Transport cost is quoted per order.

Order or ask for a quote on WhatsApp: +880 1303-438063.
Website: https://silage.khamarvest.com

### Company Profile / About Us (for the big text box on TradeFord, TradeKey, Alibaba-style sites)

Khamarvest Silage is a Bangladeshi supplier of corn (maize) silage for dairy cattle, beef
fattening cattle, goats and sheep. We harvest whole corn plants at the correct maturity
stage, chop them, and let them ferment naturally in airtight 50 kg bags, so farms can feed
green fodder all year, including the winter and monsoon months when fresh grass is scarce
or unsafe to feed.

Our silage sells at BDT 10 per kg in 50 kg airtight bags (BDT 500 per bag), with discounts
negotiable on bulk and recurring orders. An unopened bag keeps 6 to 12 months and should be
used within 2 to 3 days once opened. Typical daily feeding is 15 to 25 kg per dairy cow,
10 to 20 kg per fattening animal, and 1 to 2 kg per goat or sheep.

We deliver nationwide with cash on delivery, including Bogura, Pabna, Sirajganj, Natore,
Rangpur, Dinajpur, Mymensingh, Tangail, Dhaka, Gazipur, Kushtia, Jashore, Khulna, Satkhira,
Cumilla, Noakhali, Chattogram and Cox's Bazar. Transport cost is quoted per order at
confirmation.

Alongside supply, we publish free Bengali-language guides and feed-cost calculators for
farmers at https://silage.khamarvest.com, covering feeding rates, storage, seasonal feed
planning and farm economics.

We welcome enquiries from dairy and beef farms, feed traders, agricultural projects and
institutional buyers. Contact us on WhatsApp at +880 1303-438063.

### The other fields on those forms

Fill these truthfully, and leave blank anything you cannot back up. Directories reward a
complete profile, but an inflated one costs you the deal when a buyer checks.

- Website: https://silage.khamarvest.com (this field is the whole reason we are listing)
- Business type: pick what is actually true. If we chop and ferment the corn ourselves,
  "Manufacturer" or "Producer" fits. If we buy finished silage and resell, it is
  "Trading Company" or "Distributor / Wholesaler". Do not tick both to look bigger.
- Main products: Corn silage, maize silage, cattle feed, green fodder, dairy feed
- Main markets: Bangladesh (domestic). Do not tick export markets we do not serve.
- Year established, number of employees, annual revenue, production capacity: only we know
  these. Enter the real figures or leave them blank.
- Certifications, ISO, export licences: leave blank unless we hold the document.
- Contact: WhatsApp +880 1303-438063

### Product listing (for directories with a product field)
Product: Corn Silage, 50 kg airtight bag
Price: BDT 500 per bag (BDT 10 per kg)
Minimum order: negotiable, ask on WhatsApp
Payment: cash on delivery
Shelf life: 6 to 12 months unopened
Suitable for: dairy cows, beef fattening cattle, goats, sheep, buffalo

---

## 3. Facebook group posts

Most Bangladeshi farming groups delete straight sales posts, and farmers scroll past them
anyway. So each post below leads with the useful thing and mentions the product once, at the
end, as a fact rather than a pitch. Post one guide at a time, spaced a few days apart, and
reply to the comments. The comments are where the orders actually come from.

Never post the same text into several groups on the same day. Facebook treats that as spam
and can restrict the account.

### Post A: monsoon feeding (use now, it is the season)

বর্ষায় মাঠভরা সবুজ ঘাস দেখে মনে হয় খাবারের অভাব নেই, কিন্তু এই মৌসুমেই পেট ফাঁপা আর কৃমির
সমস্যা সবচেয়ে বেশি হয়। কারণ তিনটি: ভেজা কচি ঘাসে পানির ভাগ বেশি (পেট ভরে কিন্তু পুষ্টি কম),
কাদা-জমা পানিতে কৃমির লার্ভা, আর ঘরে রাখা খড়ে ছত্রাক।

কয়েকটা নিয়ম মানলেই ঝুঁকি অনেক কমে:
১. ঘাস কেটে কয়েক ঘণ্টা ছায়ায় রেখে পানি ঝরিয়ে দিন
২. খালি পেটে শুধু কচি ঘাস নয়, আগে অল্প খড় বা সাইলেজ দিন
৩. নিচু জলাবদ্ধ জমির ঘাস এড়িয়ে চলুন, গোড়া ঘেঁষে কাটবেন না
৪. খড় মাচায় তুলে রাখুন, ছত্রাক ধরা খড় ফেলে দিন

পুরো নিয়মটা বিস্তারিত লিখেছি এখানে: https://silage.khamarvest.com/blog/borshakale-gorur-khaddo-babosthapona

আমরা খামারভেস্ট সাইলেজ সরবরাহ করি (১০ টাকা কেজি, ৫০ কেজি বস্তা ৫০০ টাকা, সারাদেশে ক্যাশ অন
ডেলিভারি), তবে লেখাটা কেউ কিছু না কিনেও কাজে লাগাতে পারবেন। প্রশ্ন থাকলে কমেন্ট করুন।

### Post B: bloat emergency (high engagement, very shareable)

গরুর পেট হঠাৎ ফুলে গেলে হাতে সময় থাকে খুব কম। যা মনে রাখা দরকার:

লক্ষণ: পেটের বাম দিক অস্বাভাবিক ফুলে ওঠা, খাওয়া ও জাবর কাটা বন্ধ, অস্থিরতা, মুখে লালা।
শ্বাসকষ্ট শুরু হলে অবস্থা জরুরি।

সঙ্গে সঙ্গে: সব খাবার সরান, গরুকে শুতে না দিয়ে ধীরে হাঁটান, আর ভেটেরিনারি চিকিৎসক বা উপজেলা
প্রাণিসম্পদ অফিসে ফোন করুন।

যা করবেন না: নিজে থেকে ওষুধ, কেরোসিন, সোডা বা তেল খাওয়াবেন না, পেট ফুটো করার চেষ্টা তো
কখনোই নয়। কোনটা কোন ধরনের ব্লোটে চলে তা পরীক্ষা ছাড়া বলা যায় না, ভুলটা প্রাণঘাতী।

লক্ষণ, করণীয় আর প্রতিরোধের পুরো নিয়ম: https://silage.khamarvest.com/blog/gorur-pet-fapa-hole-koronio

আজই কাছের পশু চিকিৎসকের নম্বর ফোনে সেভ করে রাখুন। কাজে লাগবে।

### Post C: Napier vs silage (for groups full of grass growers)

নেপিয়ার ঘাস চাষ করব নাকি সাইলেজ কিনব, এই প্রশ্ন প্রায়ই আসে। সোজা উত্তর: জমি, সেচ আর ঘাস
কাটার লোক থাকলে নিজের নেপিয়ারই সবচেয়ে সস্তা। কিন্তু নেপিয়ারের তিনটা সীমা আছে যা আগে
কেউ বলে না:

১. সংরক্ষণ করা যায় না, আজ কেটে আজই খাওয়াতে হয়
২. শীতে (নভেম্বর-ফেব্রুয়ারি) বৃদ্ধি অনেক কমে যায়, ঠিক যখন সবুজ খাবার সবচেয়ে দরকার
৩. দেরিতে কাটলে কাণ্ড শক্ত হয়ে পুষ্টিগুণ পড়ে যায়

তাই বেশিরভাগ খামারির জন্য সেরা কৌশল হলো মেশানো: বর্ষা-গ্রীষ্মে নিজের নেপিয়ার, আর শীতের
ঘাটতির মাসগুলোর জন্য আগেই বস্তার সাইলেজ কিনে রাখা।

দুটোর বিস্তারিত তুলনা: https://silage.khamarvest.com/blog/nepier-ghas-vs-vutta-silage

### Post D: free tools (no product mention at all, pure goodwill)

খামারের হিসাব রাখার জন্য কয়েকটা জিনিস ফ্রি বানিয়ে রেখেছি, কোনো সাইন-আপ বা টাকা লাগে না,
শুধু প্রিন্ট করে গোয়ালে টানিয়ে রাখুন:

· দুধের লাভ-লস ক্যালকুলেটর: https://silage.khamarvest.com/tools/dudher-labh-calculator
· মোটাতাজাকরণ খরচ ক্যালকুলেটর: https://silage.khamarvest.com/tools/motatajakoron-khoroch-calculator
· দৈনিক খাদ্য তালিকা চার্ট: https://silage.khamarvest.com/tools/khaddo-talika-chart
· টিকা ও কৃমিনাশকের রেকর্ড শিট: https://silage.khamarvest.com/tools/tika-krimi-calendar
· মাসিক হিসাব খাতা: https://silage.khamarvest.com/tools/hishab-khata

সব টুল একসঙ্গে: https://silage.khamarvest.com/tools/

কারো কাজে লাগলে জানাবেন, আর কী দরকার সেটাও বলবেন, বানিয়ে দেব।

---

## 4. YouTube video description block

Paste at the top of every video description. The link must be in the first two lines,
because YouTube hides the rest behind "show more".

খামারভেস্ট সাইলেজ · ভুট্টা সাইলেজ ১০ টাকা কেজি, ৫০ কেজি বস্তা ৫০০ টাকা
অর্ডার (WhatsApp): https://wa.me/8801303438063
দাম, পরিমাণ ও খামারি গাইড: https://silage.khamarvest.com

দৈনিক খাওয়ানোর পরিমাণ: দুধের গাভী ১৫-২৫ কেজি, মোটাতাজাকরণের গরু ১০-২০ কেজি,
ছাগল-ভেড়া ১-২ কেজি। না-খোলা এয়ারটাইট বস্তা ৬-১২ মাস ভালো থাকে, খোলার পর ২-৩ দিনের
মধ্যে ব্যবহার করবেন।

ফ্রি ক্যালকুলেটর ও প্রিন্ট শিট: https://silage.khamarvest.com/tools/
সব খামারি গাইড: https://silage.khamarvest.com/blog/
Facebook: https://www.facebook.com/khamarvestSilage/

---

## 5. Facebook page pinned post

খামারভেস্ট সাইলেজ · খাঁটি ভুট্টা সাইলেজ, সারাদেশে ডেলিভারি

দাম: ১০ টাকা কেজি, ৫০ কেজি এয়ারটাইট বস্তা ৫০০ টাকা। পণ্য হাতে পেয়ে টাকা (ক্যাশ অন
ডেলিভারি)। বেশি নিলে দাম আলোচনাসাপেক্ষ।

অর্ডার দিতে WhatsApp-এ নাম, এলাকা আর কত বস্তা লাগবে লিখে পাঠান: https://wa.me/8801303438063

কত বস্তা লাগবে বুঝতে না পারলে ওয়েবসাইটের ক্যালকুলেটরে গরুর সংখ্যা বসিয়ে দেখুন, সঙ্গে দাম,
খাওয়ানোর নিয়ম আর ১৮টি খামারি গাইডও আছে: https://silage.khamarvest.com

কেনার আগে জেনে নিন: দুধের গাভী দিনে ১৫-২৫ কেজি, মোটাতাজাকরণের গরু ১০-২০ কেজি, ছাগল-ভেড়া
১-২ কেজি সাইলেজ খায়। নতুন গরুকে ৭ দিনে ধীরে অভ্যাস করাতে হয়।
